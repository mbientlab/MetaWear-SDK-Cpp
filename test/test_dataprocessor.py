...
class TestDataProcessingChain(TestMetaWearBase):
    ...
    def test_led_controller(self):
        ...
        def math_created(pointer):
            processors[0] = pointer
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(pointer, ComparatorOperation.EQ, 1.0, comparator_odd_handler)

        def comp_odd_created(pointer):
            processors[1] = pointer
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(processors[0], ComparatorOperation.EQ, 0.0, comparator_even_handler)

        def comp_even_created(pointer):
            processors[2] = pointer;
            events[0].set()

        math_handler = FnVoid_VoidP(math_created)
        comparator_even_handler = FnVoid_VoidP(comp_even_created)
        comparator_odd_handler = FnVoid_VoidP(comp_odd_created)
        counter_handler= FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_math_create(p, MathOperation.MODULUS, 2.0, math_handler)) 

        switch_signal= self.libmetawear.mbl_mw_switch_get_state_data_signal(self.board)
        self.libmetawear.mbl_mw_dataprocessor_counter_create(switch_signal, counter_handler)
        events[0].wait()

        event_handler = FnVoid_VoidP_Int(lambda e, s: events[1].set())
        ...
        self.libmetawear.mbl_mw_event_end_record(processors[1], event_handler)
        events[1].wait()

        event_handler = FnVoid_VoidP_Int(lambda e, s: events[2].set())
        self.libmetawear.mbl_mw_event_record_commands(processors[2])
        self.libmetawear.mbl_mw_led_stop_and_clear(self.board)
        self.libmetawear.mbl_mw_event_end_record(processors[2], event_handler)
        events[2].wait()
    ...
    def test_freefall(self):
        ...
        def threshold_processor_created(ths):
            processors.append(ths)
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(ths, ComparatorOperation.EQ, -1.0, comparator_below_handler)

        rss_handler= FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_average_create(p, 4, avg_handler))
        avg_handler = FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_threshold_create(p, ThresholdMode.BINARY, 0.5, 0.0, 
                threshold_handler))
        threshold_handler = FnVoid_VoidP(threshold_processor_created)
        comparator_below_handler = FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_comparator_create(processors[0], ComparatorOperation.EQ, 
                1.0, comparator_above_handler))
        comparator_above_handler = FnVoid_VoidP(lambda p: e.set())

        accel_signal= self.libmetawear.mbl_mw_acc_get_acceleration_data_signal(self.board)
        self.libmetawear.mbl_mw_dataprocessor_rss_create(accel_signal, rss_handler)
        e.wait()
    ...
    def test_sample_collector(self):
        ...
        sample_handler= FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_passthrough_create(p, PassthroughMode.COUNT, 0, 
                passthrough_handler))
        passthrough_handler= FnVoid_VoidP(lambda p: e.set())

        gpio_adc_signal= self.libmetawear.mbl_mw_gpio_get_analog_input_data_signal(self.board, 0, GpioAnalogReadMode.ADC)
        self.libmetawear.mbl_mw_dataprocessor_sample_create(gpio_adc_signal, 16, sample_handler)
        e.wait()
...
class TestActivityMonitorRPro(TestMetaWearBase):
    def setUp(self):
        ...
        e = threading.Event()
        self.processors = []
        def accum_processor_created(acc):
            self.processors.append(acc)
            self.libmetawear.mbl_mw_dataprocessor_buffer_create(acc, buffer_handler)

        def time_processor_created(time):
            self.processors.append(time)
            self.libmetawear.mbl_mw_dataprocessor_delta_create(time, DeltaMode.DIFFERENTIAL, 180000.0, delta_handler)

        rms_handler = FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_accumulator_create_size(p, 4, accum_handler))
        accum_handler = FnVoid_VoidP(accum_processor_created)
        buffer_handler = FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_time_create(self.processors[0], TimeMode.ABSOLUTE, 30000, 
                time_handler))
        time_handler = FnVoid_VoidP(time_processor_created)
        delta_handler = FnVoid_VoidP(lambda p: e.set())

        accel_signal= self.libmetawear.mbl_mw_acc_get_acceleration_data_signal(self.board)
        self.libmetawear.mbl_mw_dataprocessor_rms_create(accel_signal, rms_handler)
        e.wait()

        self.libmetawear.mbl_mw_datasignal_subscribe(self.processors[1], self.sensor_data_handler)
...
class TestTemperatureConversionRPro(TestMetaWearBase):
    def setUp(self):
        ...
        def f_add_processor_created(signal):
            self.fahrenheit_signal= signal;
            self.libmetawear.mbl_mw_dataprocessor_math_create(self.temp_signal, MathOperation.ADD, 273.15, k_add_handler)

        def k_add_processor_created(signal):
        ...
        f_mult_handler= FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_math_create(p, MathOperation.DIVIDE, 10.0, f_divide_handler))
        f_divide_handler= FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_dataprocessor_math_create(p, MathOperation.ADD, 32.0, f_add_handler))
        f_add_handler= FnVoid_VoidP(f_add_processor_created)
        k_add_handler= FnVoid_VoidP(k_add_processor_created)
        ...
        self.libmetawear.mbl_mw_dataprocessor_math_create(self.temp_signal, MathOperation.MULTIPLY, 18.0, f_mult_handler)
        e.wait()
...
class TestAccAxisProcessing(TestMetaWearBase):
    ...
    def test_acc_z_pulse_setup(self):
        ...
        def pulse_created(pulse):
            self.libmetawear.mbl_mw_datasignal_subscribe(pulse, self.sensor_data_handler)
            e.set()

        e = threading.Event()

        pulse_handler = FnVoid_VoidP(pulse_created)

        acc_z_signal = self.libmetawear.mbl_mw_datasignal_get_component(self.acc_signal, Const.ACC_ACCEL_Z_AXIS_INDEX)
        self.libmetawear.mbl_mw_dataprocessor_pulse_create(acc_z_signal, PulseOutput.AREA, 1.0, 16, pulse_handler)
        e.wait()
    ...
    def test_acc_y_threshold(self):
        ...
        e = threading.Event()
        def comp_created(comp):
            self.libmetawear.mbl_mw_datasignal_subscribe(comp, self.sensor_data_handler)
            e.set()

        ths_created= FnVoid_VoidP(lambda t: self.libmetawear.mbl_mw_dataprocessor_comparator_create(t, ComparatorOperation.EQ, 1.0, comp_created))
        comp_created= FnVoid_VoidP(comp_created)

        acc_y_signal = self.libmetawear.mbl_mw_datasignal_get_component(self.acc_signal, Const.ACC_ACCEL_Y_AXIS_INDEX)
        self.libmetawear.mbl_mw_dataprocessor_threshold_create(acc_y_signal, ThresholdMode.BINARY, 1.0, 0.0, ths_created)
        e.wait()
...
class TestGpioAdcPulse(TestMetaWearBase):
    def setUp(self):
        ...
        def pulse_processor_created(pulse):
            self.pulse_signal= pulse
            self.libmetawear.mbl_mw_datasignal_subscribe(pulse, self.sensor_data_handler)

            e.set()

        pulse_handler= FnVoid_VoidP(pulse_processor_created)
        gpio_adc_signal= self.libmetawear.mbl_mw_gpio_get_analog_input_data_signal(self.board, 0, GpioAnalogReadMode.ADC)
        self.libmetawear.mbl_mw_dataprocessor_pulse_create(gpio_adc_signal, PulseOutput.PEAK, 500.0, 10, pulse_handler)
...
class TestGpioFeedbackSetup(TestMetaWearBase):
    def setUp(self):
        ...
        def passthrough_processor_created(signal):
            self.offset_passthrough= signal
            self.libmetawear.mbl_mw_dataprocessor_math_create(self.gpio_abs_ref_signal, MathOperation.SUBTRACT, 0.0, handlers["math"])

        def math_processor_created(signal):
            self.abs_ref_offset= signal
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(signal, ComparatorOperation.GT, 0.0, handlers["gt_comparator"])

        def gt_comparator_processor_created(signal):
            self.gt_comparator= signal
            self.libmetawear.mbl_mw_dataprocessor_counter_create(signal, handlers["gt_counter"])

        def gt_counter_processor_created(signal):
            self.gt_comparator_counter= signal
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(signal, ComparatorOperation.EQ, 16.0, handlers["gt_counter_comparator"])

        def gt_counter_comparator_processor_created(signal):
            self.gt_comparator_counter_comparator= signal
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(self.abs_ref_offset, ComparatorOperation.LTE, 0.0, handlers["lte_comparator"])

        def lte_comparator_processor_created(signal):
            self.lte_comparator= signal
            self.libmetawear.mbl_mw_dataprocessor_counter_create(signal, handlers["lte_counter"])

        def lte_counter_processor_created(signal):
            self.lte_comparator_counter= signal
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(signal, ComparatorOperation.EQ, 16.0, handlers["lte_counter_comparator"])

        def lte_counter_comparator_processor_created(signal):
            e = threading.Event()
            event_handler = FnVoid_VoidP_Int(lambda event, s: e.set())
            ...
            self.libmetawear.mbl_mw_event_end_record(self.offset_passthrough, event_handler)
            e.wait()

            e.clear()
            self.libmetawear.mbl_mw_event_record_commands(self.gt_comparator)
            self.libmetawear.mbl_mw_dataprocessor_counter_set_state(self.lte_comparator_counter, 0)
            self.libmetawear.mbl_mw_event_end_record(self.gt_comparator, event_handler)
            e.wait()
            ...
            self.libmetawear.mbl_mw_event_end_record(self.gt_comparator_counter_comparator, event_handler)
            e.wait()

            e.clear()
            self.libmetawear.mbl_mw_event_record_commands(self.lte_comparator)
            self.libmetawear.mbl_mw_dataprocessor_counter_set_state(self.gt_comparator_counter, 0)
            self.libmetawear.mbl_mw_event_end_record(self.lte_comparator, event_handler)
            e.wait()

            e.clear()
            self.libmetawear.mbl_mw_event_record_commands(signal)
            self.libmetawear.mbl_mw_dataprocessor_passthrough_set_count(self.offset_passthrough, 1)
            self.libmetawear.mbl_mw_event_end_record(signal, event_handler)
            e.wait()

            outer.set()

        handlers["passthrough"] = FnVoid_VoidP(passthrough_processor_created)
        handlers["math"] = FnVoid_VoidP(math_processor_created)
        handlers["gt_comparator"] = FnVoid_VoidP(gt_comparator_processor_created)
        handlers["gt_counter"]= FnVoid_VoidP(gt_counter_processor_created)
        handlers["gt_counter_comparator"]= FnVoid_VoidP(gt_counter_comparator_processor_created)
        handlers["lte_comparator"]= FnVoid_VoidP(lte_comparator_processor_created)
        handlers["lte_counter"]= FnVoid_VoidP(lte_counter_processor_created)
        handlers["lte_counter_comparator"]= FnVoid_VoidP(lte_counter_comparator_processor_created)
        ...
        self.libmetawear.mbl_mw_dataprocessor_passthrough_create(self.gpio_abs_ref_signal, PassthroughMode.COUNT, 0, handlers["passthrough"])
        outer.wait()
...
class TestPassthroughSetCount(TestMetaWearBase):
    ...
    def test_valid_set_count(self):
        self.libmetawear.mbl_mw_dataprocessor_passthrough_create(self.baro_pa_signal, PassthroughMode.COUNT, 10, self.processor_handler)
...
class TestAccumulatorSetSum(TestMetaWearBase):
    ...
    def test_invalid_set_count(self):
        self.libmetawear.mbl_mw_dataprocessor_time_create(self.baro_pa_signal, TimeMode.DIFFERENTIAL, 30000, self.processor_handler)
        self.events["processor"].wait()
...
class TestCounterSetCount(TestMetaWearBase):
    ...
    def test_invalid_set_count(self):
        self.libmetawear.mbl_mw_dataprocessor_comparator_create(self.baro_pa_signal, ComparatorOperation.LT, 101325.0, self.processor_handler)
        self.events["processor"].wait()
...
class TestAverageReset(TestMetaWearBase):
    ...
    def test_invalid_reset(self):
        self.libmetawear.mbl_mw_dataprocessor_pulse_create(self.baro_pa_signal, PulseOutput.AREA, 101325.0, 64, self.processor_handler)
        self.events["processor"].wait()
...
class TestDeltaSetPrevious(TestMetaWearBase):
    ...
    def test_valid_set_previous(self):
        self.libmetawear.mbl_mw_dataprocessor_delta_create(self.baro_pa_signal, DeltaMode.DIFFERENTIAL, 25331.25, self.processor_handler)
        self.events["processor"].wait()
    ...
    def test_invalid_set_previous(self):
        self.libmetawear.mbl_mw_dataprocessor_math_create(self.baro_pa_signal, MathOperation.DIVIDE, 1000.0, self.processor_handler)
        self.events["processor"].wait()
...
class TestThreshold(TestMetaWearBase):
    ...
    def test_valid_set_count(self):
        ...
        self.libmetawear.mbl_mw_dataprocessor_threshold_create(signal, ThresholdMode.BINARY, 57.0, 0.0, self.processor_handler)
        self.events["processor"].wait()
...
class TestThreeAxisRightShift(TestMetaWearBase):
    def __init__(self, *args, **kwargs):
        ...
        self.boardType= TestMetaWearBase.METAWEAR_ENV_BOARD
        self.processor_created_fn= FnVoid_VoidP(self.processor_created)
        self.logger_created_fn= FnVoid_VoidP(self.logger_created)

    def test_acc_rshift(self):
        e = threading.Event()
        handlers= {
            "rshift": FnVoid_VoidP(lambda p: self.libmetawear.mbl_mw_datasignal_log(p, handlers["rshift_logger"])),
            "rshift_logger": FnVoid_VoidP(lambda p: e.set())
        }

        processor_created_fn= FnVoid_VoidP(self.processor_created)

        signal= self.libmetawear.mbl_mw_acc_get_acceleration_data_signal(self.board)
        self.libmetawear.mbl_mw_dataprocessor_math_create(signal, MathOperation.RSHIFT, 8, handlers["rshift"])
        e.wait()
...
class TestBmm150DataProcessor(TestMetaWearBase):
    ...
    def test_number_to_firmware(self):
        ...
        e = threading.Event()
        rss_created_ptr= FnVoid_VoidP(lambda rms: self.libmetawear.mbl_mw_dataprocessor_comparator_create(rms, ComparatorOperation.GT, 250.0, 
                comp_created_ptr))
        comp_created_ptr= FnVoid_VoidP(lambda comp: e.set())
...
class TestDataProcessorTimeout(TestMetaWearBase):
    ...
    def test_timeout_no_state(self):
        dataprocessor_created_fn= FnVoid_VoidP(self.dataprocessor_created)
...
class TestMultiComparatorRedirect(TestMetaWearBase):
    ...
    def test_modify(self):
        ...
        self.libmetawear.mbl_mw_dataprocessor_rss_create(acc_signal, self.processor_handler)
        self.events["processor"].wait()

        self.events["processor"].clear()
        self.libmetawear.mbl_mw_dataprocessor_comparator_create(self.processors[0], ComparatorOperation.GT, 2.0, self.processor_handler)
        self.events["processor"].wait()
...
class TestMultiComparator(TestMetaWearBase):
    ...
    def test_reference(self):
        ...
        self.libmetawear.mbl_mw_dataprocessor_multi_comparator_create(adc_signal, ComparatorOperation.GTE, ComparatorMode.REFERENCE, references, 
                len(references), self.processor_handler)
        self.events["processor"].wait()
...
class TestMathProcessor(TestMetaWearBase):
    def setUp(self):
        ...
        self.libmetawear.mbl_mw_dataprocessor_math_create(temp_signal, MathOperation.CONSTANT, 25.0, self.processor_handler)
        self.events["processor"].wait()
...
class TestSensorFusionLimiter(TestMetaWearBase):
    ...
    def test_quaternion(self):
        ...
        self.libmetawear.mbl_mw_dataprocessor_time_create(signal, TimeMode.ABSOLUTE, 20, self.processor_handler)
        self.events["processor"].wait()

        self.libmetawear.mbl_mw_datasignal_log(self.processors[0], self.logger_created)
