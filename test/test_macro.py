...
class TestMacro(TestMetaWearBase):
    ...
    def test_led_on_boot(self):
        e = threading.Event()
        callback = FnVoid_VoidP_Int(lambda board, status: e.set())
        ...
        self.libmetawear.mbl_mw_macro_end_record(self.board, callback)
    ...
    def test_freefall_on_boot(self):
        ...
        def threshold_processor_created(pointer):
            threshold_signal= pointer
            self.comparator_below_handler = FnVoid_VoidP(lambda pointer: self.libmetawear.mbl_mw_dataprocessor_comparator_create(threshold_signal, 
                ComparatorOperation.EQ, 1.0, comparator_above_handler))
            comparator_above_handler = FnVoid_VoidP(lambda pointer: events[1].set())
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(pointer, ComparatorOperation.EQ, -1.0, self.comparator_below_handler)

        rss_handler= FnVoid_VoidP(lambda pointer: self.libmetawear.mbl_mw_dataprocessor_average_create(pointer, 4, avg_handler))
        avg_handler= FnVoid_VoidP(lambda pointer: self.libmetawear.mbl_mw_dataprocessor_threshold_create(pointer, ThresholdMode.BINARY, 0.5, 0.0,  
                threshold_handler))
        threshold_handler= FnVoid_VoidP(threshold_processor_created)
        callback = FnVoid_VoidP_Int(lambda board, status: events[0].set())
    ...
    def test_led_controller(self):
        ...
        def comp_odd_created(pointer):
            ...
            self.libmetawear.mbl_mw_dataprocessor_comparator_create(control_signals[2], ComparatorOperation.EQ, 0.0, comparator_even_handler)
        ...
        self.libmetawear.mbl_mw_led_write_pattern(self.board, byref(pattern), LedColor.BLUE)
        self.libmetawear.mbl_mw_led_play(self.board)
        self.libmetawear.mbl_mw_event_end_record(control_signals[0], event_handler)
        events[1].wait()
        ...
        self.libmetawear.mbl_mw_event_end_record(control_signals[1], event_handler)
        events[2].wait()

        callback = FnVoid_VoidP_Int(lambda board, status: events[3].set())
        self.libmetawear.mbl_mw_macro_end_record(self.board, callback)
        events[3].wait()
